...
library(cowplot)
working_dir=getwd()
source(paste0(working_dir,"/../../lausanne_postdoc/jokela_lab_info/Treemix/Tree_Space_Analysis_Consensus/plotting_funcs.R"))

poolnames = read.csv(paste0(working_dir,"/../../lausanne_postdoc/jokela_lab_info/Treemix/Tree_Space_Analysis_Consensus/pops.lst.txt"), sep = " ", header = F)
...
all_trees = NULL
p_values = NULL
for (i in list.files("../../lausanne_postdoc/jokela_lab_info/Treemix/whole_genome_split_out/", pattern = ".treeout.gz")[c(-18,-19)]){
  name = str_remove(str_remove(i, "random_sample_without_replacement_"), ".treeout.gz")
  tree = read.table(file.path("../../lausanne_postdoc/jokela_lab_info/Treemix/whole_genome_split_out", i), skip = 1)
...
#Read in the tree structure - one of the trees after confirming they represent the same consensus structure - see treespace analysis
treemixobj = read_treemix(paste0(working_dir,"/../../lausanne_postdoc/jokela_lab_info/Treemix/whole_genome_split_out/random_sample_without_replacement_081"))
```
...
p = p + annotation_custom(ggplotGrob(empty_pie), xmin = 0.009, xmax = 0.016, 
                          ymin = 0.78, ymax = 1.00)
p
```

## Plot on a map - Figure 2C

```{r , fig.height=6, fig.width=8, warning=FALSE}

coords <- data.frame(
  pop = c("Alexandrina", "Middleton", "Paringa", "Selfe", "Mapourika"),
  lat = c(-43.95, -44.28, -43.7, -43.24, -43.32),
  lon = c(170.46, 169.84, 169.4, 171.5, 170.18),
  color = c("orange", "gold", "purple", "darkgreen", "cornflowerblue")
)

inc= tree[grepl("Selfe", tree$V5), c(1,2,6)]
colnames(inc) = c("V6", "V5", "source")
out = tree[grepl("Selfe", tree$V6), c(1,2,5)]
colnames(out) = c("V6", "V5", "source")

mig = rbind(inc,out) %>% mutate(V6 = sapply(str_split(V6, "_"), .subset,1)) %>% mutate(V5 = sapply(str_split(V5, "_"), .subset,1)) %>% group_by(V6, V5) %>% summarise(strength = mean(source))

mig[mig$V6 == "Alex",]$V6 = "Alexandrina"
mig[mig$V5 == "Alex",]$V5 = "Alexandrina"

mig_xy <- mig %>%
  left_join(coords, by = c("V5" = "pop")) %>%
  rename(x = lon, y = lat) %>%
  left_join(coords, by = c("V6" = "pop")) %>%
  rename(xend = lon, yend = lat)


nz <- ne_countries(scale = "medium",
                   country = "New Zealand",
                   returnclass = "sf")

south_island <- st_crop(
  nz,
  xmin = 167, xmax = 173,
  ymin = -45, ymax = -42.5
)

pdf(paste0(working_dir,"/../Figures/TreeMix_as_a_Map.pdf"), height = 6, width = 6)
ggplot() +
    # subtle basemap
    geom_sf(data = south_island,
            fill = "grey95",
            color = "grey70",
            linewidth = 0.2) +
    ...
    geom_point(
        ...
        size = 3,
        stroke = 1
    ) + geom_label_repel(data = coords, aes(lon, lat, label= pop), nudge_y = 1) +
    
    scale_size(range = c(0.6, 3), guide = "none") +
    scale_color_manual(
        values = c("orange", "cornflowerblue", "gold", "purple", "darkgreen")
    ) + 
    coord_sf(
        xlim = c(167, 173),
        ylim = c(-45, -42.5),
    ...
    theme_void() + labs(colour = "Source Lake") + 
  theme_bw() + 

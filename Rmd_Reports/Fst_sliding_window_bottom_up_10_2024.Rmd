---
title:  "Fst sliding analysis  - analysis of divergent regions"
...
for_fst = temp3 %>% 
  mutate(label2 = case_when(label %in% c("Alexandrina_Paringa", "Alexandrina_Mapourika", "Middleton_Paringa", "Mapourika_Middleton") ~ "West vs South Canterbury", 
                            label %in% c("Alexandrina_Selfe", "Middleton_Selfe") ~ "South Canterbury vs Selfe", 
                            label %in% c("Mapourika_Selfe", "Paringa_Selfe") ~ "West vs Selfe", 
                            label == "Alexandrina_Middleton" ~ "within South Canterbury",  label == "Mapourika_Paringa" ~ "within the West"),
         Comparison = if_else(
           (category1 == "West vs South Canterbury" & West_vs_East %in% c("", "Introgressing into Selfe from Alexandrina", "Introgressing into Selfe from Middleton", "Introgressing into Selfe from Paringa", "Introgressing into Selfe from Mapourika", "Introgressing to Selfe from South Canterbury", "Introgressing to Selfe from West", "Introgressing to Selfe from both")), paste(category1,West_vs_East), category1)) 
for_fst$Comparison = factor(for_fst$Comparison, levels = c("All comparisons undifferentiated", "All comparisons differentiated","Alexandrina vs all", "Middleton vs all", "Selfe vs all", "Mapourika vs all", "Paringa vs all", "between only two lakes", "Selfe vs South Canterbury", "Selfe vs West", "within South Canterbury",  "within the West", "West vs South Canterbury ", "West vs South Canterbury Introgressing into Selfe from Alexandrina", "West vs South Canterbury Introgressing into Selfe from Middleton", "West vs South Canterbury Introgressing into Selfe from Paringa", "West vs South Canterbury Introgressing into Selfe from Mapourika", "West vs South Canterbury Introgressing to Selfe from South Canterbury", "West vs South Canterbury Introgressing to Selfe from West", "West vs South Canterbury Introgressing to Selfe from both", "Other"))
...
pdf("../Figures/Whole_genome_subdivided_FST.pdf", width = 16, height = 7)
p4
dev.off()
```
...
p4 = for_fst %>% 
    ggplot(aes(label2, Fst, fill = Comparison)) + 
    geom_violin()  + 
    scale_fill_manual(values = c("azure4", "salmon" , "#A6CEE3FF", "darkorange", "plum","darkcyan")) + theme(axis.text.x = element_text(size = 13, angle = 45, hjust = 1, vjust = 1), plot.margin = margin(10, 10, 10, 50), axis.text.y = element_text(size = 11),legend.text = element_text(size = 11), legend.key.size = unit(1, "cm"), legend.key = element_rect(colour = NA, fill = NA)) + labs(x = "") + guides(fill=guide_legend(ncol=1)) + ylab(expression(F[ST])) + labs(fill = "Genomic regions")

pdf("../Figures/Whole_genome_subdivided_FST.2.pdf", width = 16, height = 6)
p4
dev.off()
...
p1 = ggplot(temp,aes(color,
    ...
    labs(x = "", y = "Nucleotide diversity (Theta_pi)", fill = F[ST] ~ "outlier")  +
    stat_compare_means(aes(group = color),
                       comparisons = lev,  # defined above
                       method = "t.test",        # or "t.test"
                       label = "p.signif") + scale_fill_manual(values = colors2) + theme(legend.position = "top", axis.text.x = element_blank()) + facet_grid(~factor(variable2, levels = c("South Canterbury", "Selfe", "West")))
...
p2 = ggplot(subset_combined) +
    ...
    theme(axis.text.x = element_blank(), 
          ...
          plot.background = element_blank(), strip.text = element_blank(), axis.title.y = element_blank(), plot.margin = unit(c(0, 0, 0, 0), "cm"), axis.text.y = element_text(size = 11,margin = margin(t = 10, b = 10)),legend.text = element_text(size = 11)) + scale_fill_manual(values = c("white",brewPalette(n = 21))) + guides(fill=guide_legend(ncol=1))
...
pdf("../Figures/GO_results.pdf", height = 10, width = 11)
p
dev.off()
```
